...
export const AdvancedUtilitiesSuite: React.FC<AdvancedUtilitiesSuiteProps> = ({
  ...
  const [pdfValidationMsg, setPdfValidationMsg] = useState<string | null>(null);

  const handleGeneratePdf = () => {
    setPdfValidationMsg(null);
    if (!victimName || !scammerAccount || !scamTimeline) {
      setPdfValidationMsg('Vui lòng điền các trường cốt lõi (Tên nạn nhân, STK kẻ gian, Mốc thời gian) để xuất đơn thư!');
      return;
    }
          ...
          {activeSubTab === 'pdf_generator' && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-slate-950 rounded-2xl border border-slate-800 p-5">
                ...
                {pdfValidationMsg && (
                  <div className="md:col-span-2 p-3 bg-rose-950/40 border border-rose-800/60 rounded-xl text-xs text-rose-300 flex items-center space-x-2">
                    <AlertTriangle className="w-4 h-4 text-rose-400 flex-shrink-0" />
                    <span>{pdfValidationMsg}</span>
                  </div>
                )}
