...
import { UserProfile, SocialPost, ReactionType, FriendUser } from '../types';
...
const INITIAL_SOCIAL_LEADERBOARD: LeaderboardUser[] = [
  { rank: 1, userId: 'u-1', name: 'Bác Nguyễn Văn Thành', xp: 11200, tier: 'Hiệp Sĩ An Ninh Mạng', streak: 120, avatar: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=100&auto=format&fit=crop&q=80', followers: 2150, likes: 4820, shares: 340, socialBadge: '👵 Vệ Binh Cao Niên Mẫu', isFollowing: true },
  { rank: 2, userId: 'u-2', name: 'Hoàng Minh Tuấn', xp: 9850, tier: 'Hiệp Sĩ An Ninh Mạng', streak: 98, avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=80', followers: 1420, likes: 3290, shares: 285, socialBadge: '🛡️ KOL Phân Tích Phishing', isFollowing: true },
  { rank: 3, userId: 'u-3', name: 'Nguyễn Thị Mai Anh', xp: 7420, tier: 'Dân Chơi Không Bẫy', streak: 75, avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=80', followers: 980, likes: 2150, shares: 190, socialBadge: '⚖️ Chuyên Gia Pháp Lý Scam', isFollowing: true },
  { rank: 4, userId: 'u-4', name: 'Phạm Quỳnh Nga', xp: 4500, tier: 'Dân Chơi Không Bẫy', streak: 45, avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&auto=format&fit=crop&q=80', followers: 610, likes: 1420, shares: 95, socialBadge: '🌸 Vệ Binh Cần Thơ', isFollowing: false },
  { rank: 5, userId: 'u-5', name: 'Lê Minh Khôi', xp: 3100, tier: 'Báo Động Đỏ', streak: 30, avatar: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=100&auto=format&fit=crop&q=80', followers: 430, likes: 980, shares: 62, socialBadge: '⚡ Vệ Binh GenZ', isFollowing: false },
];
...
export const LeaderboardView: React.FC<LeaderboardViewProps> = ({
  ...
  const [tab, setTab] = useState<'national' | 'global' | 'social' | 'growth'>('national');
  ...
  const handleToggleFollow = (rankOrId: number | string) => {
    if (onToggleFollowUser && typeof rankOrId === 'string') {
      onToggleFollowUser(rankOrId);
    }
    setSocialList(prev => prev.map(item => {
      if (item.rank === rankOrId || item.userId === rankOrId) {
  ...
  const sortedXpList = [...NATIONAL_LEADERBOARD.map(u => u.xp), currentXp].sort((a, b) => b - a);
  const myCalculatedRank = sortedXpList.indexOf(currentXp) + 1;

  const currentUserEntry: LeaderboardUser = {
    rank: myCalculatedRank,
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 space-y-6 animate-fadeIn">
      {/* Header Banner */}
      <div className="cyber-card-neon p-6 sm:p-8 rounded-3xl border-2 border-cyan-500/50 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center space-x-4 relative z-10">
          ...
          <div>
            <div className="inline-flex items-center space-x-1.5 px-3 py-0.5 rounded-full cyber-badge-gold text-[10px] font-extrabold uppercase mb-1">
              ...
              <span>BẢNG XẾP HẠNG & MẠNG XÃ HỘI VỆ BINH</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-white text-glow-gold">Hiệp Sĩ An Ninh Mạng</h1>
            <p className="text-xs sm:text-sm text-slate-300 mt-1 font-medium">
              Thi đua phòng thủ, chia sẻ cảnh báo lừa đảo thời gian thực & bảo vệ gia đình!
            </p>
          </div>
        </div>
        ...
        <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-cyan-500/40 shrink-0 relative z-10 shadow-lg flex-wrap gap-1">
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center space-x-1.5 cursor-pointer ${
              tab === 'growth' ? 'bg-gradient-to-r from-amber-500 to-purple-600 text-white shadow-md font-black' : 'text-slate-400 hover:text-white'
            }`}
          >
            <Gift className="w-3.5 h-3.5 text-amber-300" />
            <span>👨‍👩‍👧‍👦 Gia Đình & Certificate</span>
          </button>
        </div>
      </div>
      ...
      {/* VIEW 1 & 2: LEADERBOARD TABLES (national / global) */}
      {(tab === 'national' || tab === 'global') && (
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-3xl bg-gradient-to-r from-cyan-950/80 via-slate-900 to-indigo-950/80 border-2 border-cyan-500/60 shadow-xl flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-slate-400">Xếp hạng của bạn</p>
            </div>
          </div>
      ...
      {/* VIEW 3: FULL SOCIAL NETWORK FEED VIEW */}
      {tab === 'social' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Main Social Feed (Posts, Composer, Comments, Reactions, Share) */}
          ...
          {/* Right Sidebar Widget: Top Social Influencer List */}
          <div className="lg:col-span-4 space-y-4">
            <div className="cartoon-card p-5 bg-slate-900/90 border-slate-800 space-y-4 shadow-xl sticky top-20">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-xs font-black text-white font-mono uppercase tracking-wider flex items-center space-x-2">
                  <Award className="w-4 h-4 text-pink-400 animate-pulse" />
                  <span>TOP KREATORS AN NINH MẠNG</span>
                </h3>
              ...
              <div className="space-y-3">
                {socialList.map((user) => (
                  <div key={user.rank} className="p-3 bg-slate-950/80 rounded-2xl border border-slate-850 flex items-center justify-between text-xs">
                    <div className="flex items-center space-x-2.5">
                      ...
                      />
                      <div>
                        <p className="font-bold text-white text-[11px] truncate max-w-[110px]">{user.name}</p>
                        <p className="text-[9px] text-pink-400 font-mono font-bold flex items-center space-x-1">
                          <Heart className="w-2.5 h-2.5 text-pink-400 fill-pink-400" />
                          <span>{user.likes} Lượt thích</span>
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => handleToggleFollow(user.userId || user.rank)}
                      className={`px-2.5 py-1 rounded-lg text-[10px] font-mono font-bold transition-all flex items-center space-x-1 cursor-pointer ${
                        user.isFollowing
                    ...
                    >
                      ...
                      <span>{user.isFollowing ? 'Theo dõi' : 'Follow'}</span>
                    </button>
                  </div>
                ))}
              </div>
      ...
      {/* VIEW 4: FAMILY CIRCLE & REFERRAL & DNA SUITE */}
      {tab === 'growth' && (
        <SocialGrowthSuite
