...
export const RoadmapSnakePath: React.FC<RoadmapSnakePathProps> = ({
  ...
  // ViSEF Survey Question to Sector Mapping (1-to-1 correlation for ViSEF 2026 research)
  const SECTOR_QUESTION_MAP: Record<string, { qKey: string; correct: string }> = {
    'sector-1': { qKey: 'q1', correct: 'C' },
    'sector-2': { qKey: 'q2', correct: 'E' },
    'sector-3': { qKey: 'q3', correct: 'D' },
    'sector-4': { qKey: 'q4', correct: 'C' },
    'sector-5': { qKey: 'q5', correct: 'E' },
    'sector-6': { qKey: 'q6', correct: 'F' },
    'sector-7': { qKey: 'q7', correct: 'E' },
    'sector-8': { qKey: 'q8', correct: 'C' },
    'sector-9': { qKey: 'q9', correct: 'D' },
    'sector-10': { qKey: 'q10', correct: 'E' },
    'sector-11': { qKey: 'q11', correct: 'C' },
    'sector-12': { qKey: 'q12', correct: 'D' },
  };

  // ViSEF Survey Gate State: User must complete ViSEF survey to unlock personalized training roadmap
  const [hasCompletedSurvey, setHasCompletedSurvey] = useState<boolean>(() => {
    try {
      const latest = localStorage.getItem('visef_my_latest_survey');
      const submission = localStorage.getItem('scamguard_survey_submission');
      const completed = localStorage.getItem('scamguard_survey_completed') || localStorage.getItem('visef_survey_completed');
      const result = localStorage.getItem('scamguard_last_survey_result');
      return Boolean(latest || submission || completed === 'true' || result);
    } catch (e) {
      return false;
    }
  });
  const [personalSurveyData, setPersonalSurveyData] = useState<any>(() => {
    try {
      const raw = localStorage.getItem('scamguard_survey_submission') || 
                  localStorage.getItem('visef_my_latest_survey') || 
                  localStorage.getItem('scamguard_last_survey_result');
  ...
  // Dynamic sectors state sorted strictly by personalized failure rate
  const [sectorsSorted, setSectorsSorted] = useState<typeof CAMPAIGN_SECTORS>(() => CAMPAIGN_SECTORS);

  // Listen to realtime survey events across components
  useEffect(() => {
    const handleSurveyDone = (e?: any) => {
      try {
        const raw = localStorage.getItem('scamguard_survey_submission') || 
                    localStorage.getItem('visef_my_latest_survey') || 
                    localStorage.getItem('scamguard_last_survey_result');
        const parsed = e?.detail || (raw ? JSON.parse(raw) : null);
        setPersonalSurveyData(parsed);
    ...
    window.addEventListener('visef_survey_updated', handleSurveyDone);
    window.addEventListener('scamguard_survey_completed', handleSurveyDone);
    return () => {
      window.removeEventListener('visef_survey_updated', handleSurveyDone);
      window.removeEventListener('scamguard_survey_completed', handleSurveyDone);
    };
  }, []);

  // Quick Demo Survey Action: Generates realistic survey response and immediately generates personalized roadmap
  const handleQuickDemoSurvey = () => {
    const mockPayload = {
      participantName: userProfile.name || 'Học viên ViSEF',
      isAnonymous: false,
      schoolName: 'THPT Chuyên Lê Hồng Phong',
      className: 'Khối 11 Tin',
      demographicGroup: userProfile.mode === 'senior' ? 'SENIOR' : userProfile.mode === 'teen' ? 'STUDENT' : 'WORKER',
      location: 'Hà Nội',
      surveyResponses: {
        everEncounteredScam: true,
        pastLossOrNearMiss: 'LOST_MONEY',
        preConfidenceScore: 38,
        biggestFearTactic: 'TELEGRAM_INCOME',
        experiencedSectors: ['KV5', 'KV9', 'KV7'],
        experienceAnswers: {
          q1: 'B_SAFE',
          q2: 'B_SAFE',
          q3: 'C_NEAR_MISS',
    ...
    try {
      localStorage.setItem('scamguard_survey_submission', JSON.stringify(mockPayload));
      localStorage.setItem('visef_my_latest_survey', JSON.stringify(mockPayload));
    ...
    setPersonalSurveyData(mockPayload);
    setHasCompletedSurvey(true);
    window.dispatchEvent(new CustomEvent('visef_survey_updated', { detail: mockPayload }));
    window.dispatchEvent(new CustomEvent('scamguard_survey_completed', { detail: mockPayload }));
  ...
  // Helper function to calculate personal & community failure/error rates dynamically for each sector
  const calculateSectorFailureRate = (
    sectorId: string,
    analytics: any,
    surveyData?: any
  ): number => {
    const currentSurvey = surveyData || personalSurveyData;
    let personalPenalty = 0;

    if (currentSurvey) {
      const responses = currentSurvey.surveyResponses || currentSurvey;
      const trapAnswers = responses.trapAnswers || {};
      const experienceAnswers = responses.experienceAnswers || {};
      const qInfo = SECTOR_QUESTION_MAP[sectorId];

      if (qInfo) {
        // 1. Trap answer logic (Part 2 of ViSEF survey):
        const userTrapAnswer = trapAnswers[qInfo.qKey];
        if (userTrapAnswer) {
          if (userTrapAnswer !== qInfo.correct) {
            personalPenalty += 45; // Selected trap answer! High vulnerability
          } else {
            personalPenalty -= 10; // Avoided the trap safely
          }
        }

        // 2. Real-world experience (Part 1 of ViSEF survey):
        const userExp = experienceAnswers[qInfo.qKey];
        if (userExp === 'D_VICTIM') {
          personalPenalty += 35; // Already suffered damage
        } else if (userExp === 'C_NEAR_MISS') {
          personalPenalty += 25; // Close call
        } else if (userExp === 'A_NEVER') {
          personalPenalty += 12; // Unexposed blind spot
        } else if (userExp === 'B_SAFE') {
          personalPenalty -= 5;
        }
      }

      // 3. Demographic vulnerability weighting
      const demoGroup = currentSurvey.demographicGroup || responses.demographicGroup;
      if (demoGroup === 'STUDENT') {
        if (sectorId === 'sector-5') personalPenalty += 20; // Telegram job
        if (sectorId === 'sector-1') personalPenalty += 15; // Gift / brand SMS
        if (sectorId === 'sector-11') personalPenalty += 15; // Public wifi
        if (sectorId === 'sector-4') personalPenalty += 10; // Online dating
      } else if (demoGroup === 'SENIOR') {
        if (sectorId === 'sector-8') personalPenalty += 25; // Hospital / emergency call
        if (sectorId === 'sector-6') personalPenalty += 20; // Police / Government impersonation
        if (sectorId === 'sector-9') personalPenalty += 20; // Deepfake family video call
        if (sectorId === 'sector-12') personalPenalty += 15; // Recovery / fake lawyer
      } else if (demoGroup === 'WORKER') {
        if (sectorId === 'sector-3') personalPenalty += 20; // COD shopping
        if (sectorId === 'sector-7') personalPenalty += 20; // Restaurant QR bill
        if (sectorId === 'sector-2') personalPenalty += 15; // Bank brand SMS
        if (sectorId === 'sector-10') personalPenalty += 15; // Web3 / token investment
      }

      // 4. Past loss penalty
      const pastLoss = responses.pastLossOrNearMiss;
      if (pastLoss === 'LOST_MONEY' && (sectorId === 'sector-5' || sectorId === 'sector-6' || sectorId === 'sector-12')) personalPenalty += 18;
      if (pastLoss === 'SHARED_OTP_PASSWORD' && (sectorId === 'sector-2' || sectorId === 'sector-8' || sectorId === 'sector-10')) personalPenalty += 18;
      if (pastLoss === 'CLICKED_SUSPICIOUS_LINK' && (sectorId === 'sector-1' || sectorId === 'sector-7' || sectorId === 'sector-3')) personalPenalty += 18;

      // 5. Explicit feared tactic
      const fearTactic = responses.biggestFearTactic;
      if (sectorId === 'sector-6' && fearTactic === 'AUTHORITY_POLICE') personalPenalty += 25;
      if (sectorId === 'sector-8' && fearTactic === 'URGENT_ACCIDENT') personalPenalty += 25;
      if (sectorId === 'sector-3' && fearTactic === 'FAKE_BILL_QR') personalPenalty += 25;
      if (sectorId === 'sector-5' && fearTactic === 'TELEGRAM_INCOME') personalPenalty += 25;
      if (sectorId === 'sector-9' && fearTactic === 'DEEPFAKE_CALL') personalPenalty += 25;
      if (sectorId === 'sector-10' && fearTactic === 'CRYPTO_WEB3') personalPenalty += 25;
    }

    const getDnaVuln = (key: string) => {
      const item = analytics?.scamDnaComparativeRadar?.find((d: any) => d.dimensionKey === key);
      return item ? item.preAppVulnerability : 45;
    };

    const getTacticPct = (key: string) => {
      const item = analytics?.fearTacticsDistribution?.find((t: any) => t.tacticKey === key);
      return item ? item.percentage : 18;
    };

    let baseRate = 45;
    switch (sectorId) {
      case 'sector-1':
        baseRate = (getDnaVuln('G') + getDnaVuln('R') + getTacticPct('TELEGRAM_INCOME')) / 3;
        break;
      case 'sector-2':
        baseRate = (getDnaVuln('R') + getDnaVuln('T')) / 2;
        break;
      case 'sector-3':
        baseRate = (getDnaVuln('C') + getTacticPct('FAKE_BILL_QR')) / 2;
        break;
      case 'sector-4':
        baseRate = (getDnaVuln('E') + getTacticPct('DEEPFAKE_CALL')) / 2;
        break;
      case 'sector-5':
        baseRate = (getDnaVuln('G') * 0.4 + getTacticPct('TELEGRAM_INCOME') * 1.5);
        break;
      case 'sector-6':
        baseRate = (getDnaVuln('A') * 0.4 + getTacticPct('AUTHORITY_POLICE') * 1.5);
        break;
      case 'sector-7':
        baseRate = (getDnaVuln('C') + getTacticPct('FAKE_BILL_QR')) / 2;
        break;
      case 'sector-8':
        baseRate = (getDnaVuln('T') * 0.4 + getTacticPct('URGENT_ACCIDENT') * 1.5);
        break;
      case 'sector-9':
        baseRate = (getDnaVuln('E') * 0.4 + getTacticPct('DEEPFAKE_CALL') * 1.5);
        break;
      case 'sector-10':
        baseRate = getDnaVuln('G');
        break;
      case 'sector-11':
        baseRate = getDnaVuln('C');
        break;
      case 'sector-12':
        baseRate = getDnaVuln('R');
        break;
      default:
        baseRate = 45;
    }
    return Math.min(99.5, Math.max(15, +(baseRate + personalPenalty).toFixed(1)));
  };

  // Re-sort sectors strictly personalized by ViSEF survey failure rates (sequential 1->12 is removed)
  useEffect(() => {
    const sorted = [...CAMPAIGN_SECTORS].sort((a, b) => {
      const rateA = calculateSectorFailureRate(a.id, analyticsData, personalSurveyData);
      const rateB = calculateSectorFailureRate(b.id, analyticsData, personalSurveyData);
      return rateB - rateA;
    });
    setSectorsSorted(sorted);

    // If survey is completed, automatically ensure activeSectorId points to the #1 priority sector or an unlocked sector
    if (hasCompletedSurvey && sorted.length > 0) {
      setActiveSectorId((currentId) => {
        const foundIdx = sorted.findIndex((s) => s.id === currentId);
  ...
  }, [analyticsData, personalSurveyData, hasCompletedSurvey]);
  ...
  const handleSelectSectorIndex = (idx: number) => {
    if (!isSectorUnlocked(idx)) return;
  ...
  // Fetch and update real-time science evaluation telemetry
  ...
  const [certifiedSectors, setCertifiedSectors] = useState<number[]>([]);
  ...
  useEffect(() => {
    const checkCerts = () => {
      try {
        ...
        if (raw) {
          ...
          const passedSecs = list
            .filter((r: any) => r.isPassed)
            .map((r: any) => r.sectorNumber);
          setCertifiedSectors(passedSecs);
        }
      } catch (e) {
        // ignore
      }
    ...
    checkCerts();
    window.addEventListener('visef_post_cert_updated', checkCerts);
    return () => window.removeEventListener('visef_post_cert_updated', checkCerts);
  }, []);
  ...
  const markNodeComplete = (nodeId: string, earnedXp: number) => {
    setCompletedNodeIds((prev) => {
      ...
      // Check if this newly completed node finishes the entire sector
      const isSectorNowComplete = currentSector.nodes.every((n) => next.includes(n.id));
      if (isSectorNowComplete && !certifiedSectors.includes(currentSector.sectorNumber)) {
        setTimeout(() => {
  ...
  const getNodeStatus = (
    ...
    // If ViSEF survey is not completed, lock all nodes
    ...
    // In Sector #1 (highest personalized risk sector), station 1 (index 0) is immediately unlocked and active!
    ...
    } else if (activeSectorIndex > 0 && nodeIndexInSector === 0) {
      // First node of subsequent sector is active if last node of previous sector is completed
  ...
  // Sector unlock status: ONLY unlocked when previous sector is 100% finished
  const isSectorUnlocked = (sectorIdx: number) => {
    if (!hasCompletedSurvey) return false;
    if (sectorIdx === 0) return true;
    const prevSector = sectorsSorted[sectorIdx - 1];
    if (!prevSector) return false;
    // Unlocked ONLY when ALL nodes in previous sector are completed
    return prevSector.nodes.every((n) => completedNodeIds.includes(n.id));
  };
  ...
  // Zig-zag offset for serpentine path inside single frame - pronounced oscillation (10 nodes)
  const getHorizontalOffset = (index: number) => {
    const pattern = [-60, 60, -60, 60, -60, 60, -60, 60, -60, 60];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-end">
              ...
              <button
                ...
                className="px-4 py-2.5 text-xs flex items-center space-x-2 rounded-2xl shadow-lg cursor-pointer font-extrabold uppercase font-mono bg-gradient-to-r from-emerald-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 text-white border border-emerald-400/40 shadow-emerald-500/20"
                title="Thi sát hạch chứng chỉ 20 câu ViSEF"
              >
                <ShieldCheck className="w-4 h-4 text-emerald-300" />
                <span>Thi Chứng Chỉ ViSEF</span>
              </button>
      ...
      >
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <span className="text-[8px] text-slate-500 font-mono block uppercase">
                  {hasCompletedSurvey
                    ? '🎯 LỘ TRÌNH CÁ NHÂN HÓA VISEF: ĐÃ SẮP XẾP THEO ĐIỂM YẾU CAO NHẤT (ĐÃ BỎ THỨ TỰ TUẦN TỰ 1->12)'
                    : '🔒 LỘ TRÌNH TẠM KHÓA: VUI LÒNG LÀM KHẢO SÁT VISEF ĐỂ NHẬN LỘ TRÌNH RIÊNG VÀ MỞ KHÓA TRẠM ĐẦU TIÊN'}
                </span>
              </div>
            </div>

            {/* Personalized ViSEF Status & Quick Action Buttons */}
            <div className="flex flex-wrap items-center gap-2 self-start lg:self-auto">
              {!hasCompletedSurvey ? (
                <button
                  type="button"
                  onClick={() => setIsSurveyModalOpen(true)}
                  className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-amber-500 via-rose-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white text-xs font-bold font-mono transition shadow-lg shadow-rose-500/25 flex items-center gap-1.5 cursor-pointer animate-pulse active:scale-95"
                  title="Làm khảo nghiệm ViSEF để mở khóa lộ trình cá nhân hóa"
                >
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-200" />
                  <span>Khảo Sát ViSEF Để Mở Khóa</span>
                </button>
              ) : (
                <div className="flex items-center gap-2 bg-slate-950 p-1 rounded-xl border border-slate-800">
                  <span className="px-2.5 py-1 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 text-[10px] font-mono font-bold flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Lộ Trình Cá Nhân Hóa ViSEF</span>
                  </span>
                  <button
                    type="button"
                    onClick={() => setIsSurveyModalOpen(true)}
                    className="px-2.5 py-1 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-slate-900 text-[10px] font-mono font-bold transition cursor-pointer"
                    title="Khảo sát lại để tái hiệu chuẩn lộ trình theo kết quả mới"
                  >
                    🔄 Khảo Sát Lại
                  </button>
                </div>
              ...
              <div className="flex items-center space-x-1.5">
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl text-xs font-bold font-mono flex items-center space-x-1 border border-slate-800 transition-all ${
                    activeSectorIndex > 0
                ...
                <button
                  disabled={
                    ...
                    !isSectorUnlocked(activeSectorIndex + 1)
                  }
                  onClick={() =>
                    ...
                    <div className="pl-1 w-full">
                      <div className="flex items-center justify-between w-full mb-2.5">
                        <div className="flex items-center space-x-1">
                          ...
                          {idx === 0 && hasCompletedSurvey && (
                            <span className="text-[6.5px] font-mono font-bold px-1 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                              TRẠM 1 MỞ
                            </span>
                          )}
                        </div>
          ...
          <div className="space-y-2 relative z-10 pt-3 border-t border-slate-800/80">
            ...
            <div className="p-3.5 rounded-2xl bg-rose-950/15 border border-rose-500/20 flex flex-col space-y-1.5 mt-3 animate-fade-in">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-1">
                ...
                <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded bg-rose-500/15 text-rose-300 border border-rose-500/25">
                  TỶ LỆ LỖI KHẢO SÁT ViSEF: {calculateSectorFailureRate(currentSector.id, analyticsData, personalSurveyData).toFixed(1)}%
                </span>
              </div>
        ...
        {/* VISEF SCIENTIFIC GATEWAY & PERSONALIZED ROADMAP CALLOUT */}
        {!hasCompletedSurvey ? (
          <div className="bg-gradient-to-r from-slate-900 via-rose-950/30 to-amber-950/30 border-2 border-amber-500/50 rounded-3xl p-5 sm:p-6 shadow-2xl relative overflow-hidden space-y-3.5 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-[10px] font-mono font-bold border border-amber-500/40 uppercase tracking-wider">
                      Yêu Cầu Chẩn Đoán Ban Đầu ViSEF
                    </span>
                    <span className="text-xs font-mono text-rose-400 font-bold">
                      Đã gỡ bỏ thứ tự cố định 1 → 12
                    </span>
                  </div>
                  <h3 className="text-base sm:text-lg font-black text-white tracking-tight mt-1">
                    Lộ Trình Đang Khóa: Vui Lòng Làm Khảo Sát ViSEF Để Mở Khóa Huấn Luyện
                  </h3>
                </div>
              </div>
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Nhằm đảm bảo tính khoa học thực nghiệm của đề tài <strong>ViSEF 2026</strong>, ScamGuard đã <strong>hủy bỏ lộ trình tuần tự 1 → 12</strong>. 
              Bạn cần thực hiện bài khảo nghiệm ViSEF (17 câu hỏi thực tế). Ngay sau khi hoàn thành, hệ thống sẽ tự động đo lường điểm yếu, <strong>xếp lộ trình cá nhân hóa riêng</strong> cho bạn và <strong>mở khóa ngay trạm đầu tiên của khu vực nguy cơ cao nhất</strong> để bạn học!
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
              <button
                type="button"
                onClick={() => setIsSurveyModalOpen(true)}
                className="px-5 py-3 rounded-xl bg-gradient-to-r from-amber-500 via-rose-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white font-bold text-xs sm:text-sm transition-all shadow-lg shadow-rose-500/25 flex items-center gap-2 cursor-pointer active:scale-95"
              >
                <FileText className="w-4 h-4" />
                <span>📝 Làm Khảo Sát ViSEF (5 Phút) Để Mở Khóa</span>
              </button>

              <button
                type="button"
                onClick={handleQuickDemoSurvey}
                className="px-4 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-cyan-300 border border-cyan-500/30 font-mono font-bold text-xs transition flex items-center gap-2 cursor-pointer"
                title="Dành cho giám khảo / người thử nghiệm muốn kiểm tra ngay tính năng cá nhân hóa lộ trình"
              >
                <Zap className="w-4 h-4 text-cyan-400" />
                <span>⚡ Kích Hoạt Nhanh Bằng Dữ Liệu Nghiên Cứu Mẫu</span>
              </button>
            </div>
          </div>
        ) : (
          <div className="bg-gradient-to-r from-slate-900 via-emerald-950/30 to-indigo-950/30 border border-emerald-500/40 rounded-3xl p-5 sm:p-6 shadow-xl relative overflow-hidden space-y-3 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center shrink-0 shadow-lg">
                  <ShieldCheck className="w-6 h-6 text-emerald-400" />
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-mono font-bold border border-emerald-500/40 uppercase tracking-wider flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-emerald-400" />
                      Lộ Trình Đã Cá Nhân Hóa Theo Khảo Sát ViSEF
                    </span>
                    <span className="text-xs font-mono text-cyan-400 font-bold">
                      Ưu Tiên #1: {sectorsSorted[0]?.title}
                    </span>
                  </div>
                  <h3 className="text-base sm:text-lg font-black text-white tracking-tight mt-1">
                    Đã Mở Khóa Trạm 1: "{sectorsSorted[0]?.nodes[0]?.title}"
                  </h3>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={() => {
                    if (sectorsSorted[0]?.nodes[0]) {
                      if (activeSectorId !== sectorsSorted[0].id) {
                        setActiveSectorId(sectorsSorted[0].id);
                      }
                      handleLaunchNode(sectorsSorted[0].nodes[0]);
                    }
                  }}
                  className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-slate-950 font-black text-xs transition-all shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 cursor-pointer active:scale-95"
                >
                  <Play className="w-3.5 h-3.5 fill-slate-950" />
                  <span>Vào Học Ngay Trạm 1 ({sectorsSorted[0]?.nodes[0]?.title})</span>
                </button>
              </div>
            </div>

            <p className="text-xs text-slate-300 leading-relaxed">
              Dựa trên bài khảo nghiệm ViSEF của bạn, hệ thống AI xác định bạn có chỉ số tổn thương cao nhất ({calculateSectorFailureRate(sectorsSorted[0]?.id, analyticsData, personalSurveyData).toFixed(1)}% rủi ro) ở <strong>{sectorsSorted[0]?.title}</strong>. 
              Các phân khu đã được tái sắp xếp theo mức độ ưu tiên giảm dần, và trạm đầu tiên đã sẵn sàng để bạn tham gia rèn luyện!
            </p>
          </div>
        )}

        {/* OPERATIONS HOLOGRAPHIC ROADMAP FIELD */}
        <div className="relative max-w-2xl mx-auto bg-[#070b13] rounded-3xl border border-slate-800 shadow-[inset_0_0_40px_rgba(0,0,0,0.8)] overflow-hidden pt-10 pb-16">
          ...
          <div className="absolute inset-0 pointer-events-none flex justify-center">
            <svg
              ...
              viewBox={`0 0 400 ${(currentSector.nodes.length + 1) * 185 + 80}`}
              fill="none"
            ...
            >
              {currentSector.nodes.slice(1).map((node, i) => {
                ...
                const prevY = 40 + i * 185 + 54;
                const x = 200 + getHorizontalOffset(i + 1);
                const y = 40 + (i + 1) * 185 + 54;

                const midY = (prevY + y) / 2;
                const controlOffset = (i % 2 === 0 ? 18 : -18);
              ...
              {/* Conduit to the 11th Sector Certification Gate */}
              {(() => {
                const lastNodeIdx = currentSector.nodes.length - 1;
                const certNodeIdx = currentSector.nodes.length;
                const prevX = 200 + getHorizontalOffset(lastNodeIdx);
                const prevY = 40 + lastNodeIdx * 185 + 54;
                const x = 200 + getHorizontalOffset(certNodeIdx);
                const y = 40 + certNodeIdx * 185 + 54;
                const midY = (prevY + y) / 2;
                const controlOffset = (lastNodeIdx % 2 === 0 ? 18 : -18);
                const pathD = `M ${prevX} ${prevY} C ${prevX + controlOffset} ${midY}, ${x - controlOffset} ${midY}, ${x} ${y}`;

                return (
                  <g key="cyber-path-to-cert-gate">
                    {isCurrentSectorFinished ? (
                      <>
                        <path d={pathD} stroke="#000" strokeWidth="8" strokeLinecap="round" opacity="0.9" />
                        <path d={pathD} stroke="#10b981" strokeWidth="12" strokeLinecap="round" opacity="0.3" className="animate-pulse" />
                        <path d={pathD} stroke="#10b981" strokeWidth="4" strokeLinecap="round" />
                        <path d={pathD} stroke="#ffffff" strokeWidth="2" strokeLinecap="round" className="cyber-conduit-flow-completed" />
                      </>
                    ) : (
                      <path d={pathD} stroke="#1e293b" strokeWidth="3" strokeDasharray="6 6" strokeLinecap="round" opacity="0.5" />
                    )}
                  </g>
                );
              })()}
          ...
          <div className="relative z-10">
            {currentSector.nodes.map((node, nIdx) => {
              ...
              const offsetPx = getHorizontalOffset(nIdx);

              return (
                <div
                  key={node.id}
                  className="relative w-full h-[185px] flex justify-center"
                >
                  {/* Positioned inside each row with horizontal offset */}
                  <div
                  ...
                  >
                    ...
                    <div
                      onClick={() => handleLaunchNode(node)}
                      className={`mt-2.5 w-[170px] sm:w-[195px] px-3.5 py-2.5 rounded-xl cursor-pointer transition-all duration-300 border backdrop-blur-md z-20 hover:scale-103 active:scale-97 text-center flex flex-col items-center relative overflow-hidden ${
                        status === 'completed'
            ...
            {/* 11TH NODE: FINAL SECTOR CERTIFICATION GATEWAY EMBEDDED IN SNAKE PATH */}
            {(() => {
              const certIdx = currentSector.nodes.length;
              const certOffsetPx = getHorizontalOffset(certIdx);
              const isCertified = certifiedSectors.includes(currentSector.sectorNumber);
              const remainingNodes = currentSector.nodes.length - sectorCompletedNodes.length;

              return (
                <div className="relative w-full h-[220px] flex justify-center mt-2">
                  <div
                    className="absolute flex flex-col items-center z-20"
                    style={{
                      top: '18px',
                      transform: `translateX(${certOffsetPx}px)`,
                    }}
                  >
                    {/* Glowing Beacon for Unlocked Gate */}
                    {isCurrentSectorFinished && (
                      <div className="absolute -inset-5 rounded-3xl bg-emerald-500/20 animate-ping opacity-40 pointer-events-none" />
                    )}

                    <button
                      type="button"
                      onClick={() => {
                        if (isCurrentSectorFinished) {
                          setIsCertificationModalOpen(true);
                        } else {
                          setBriefingNode({
                            id: `cert-gate-${currentSector.sectorNumber}`,
                            sectorNumber: currentSector.sectorNumber,
                            nodeIndex: 99,
                            title: `Cổng Sát Hạch Chứng Chỉ Khu Vực ${currentSector.sectorNumber}`,
                            subtitle: `Yêu cầu hoàn thành tất cả 10 trạm trước khi thi sát hạch 20 câu ViSEF. Hiện còn thiếu ${remainingNodes} trạm.`,
                            kind: 'lesson',
                            icon: '🎖️',
                            threatCategory: 'CHỨNG CHỈ TỐT NGHIỆP',
                            xpReward: 300,
                            estimatedMinutes: 5,
                            lessonData: {
                              id: `cert-briefing-${currentSector.sectorNumber}`,
                              title: `Quy Trình Sát Hạch Chuẩn ViSEF - Khu Vực ${currentSector.sectorNumber}`,
                              scenarioType: 'SMS_PHISHING',
                              scenarioText: `Để kích hoạt bài thi Sát Hạch 20 Câu Hỏi Chuẩn Hóa ViSEF (>50% đạt chuẩn), bạn cần vượt qua toàn bộ 10 trạm huấn luyện trong Khu Vực ${currentSector.sectorNumber}.`,
                              evidenceItems: [],
                              difficulty: 'ADVANCED',
                              tactics: [],
                              learningObjectives: [
                                `Hoàn thành ${sectorCompletedNodes.length}/${currentSector.nodes.length} trạm hiện tại`,
                                'Mở khóa bài thi sát hạch 20 câu hỏi',
                                'Đạt chuẩn để nhận chứng chỉ số ViSEF 2026',
                              ],
                              steps: [],
                            },
                          });
                      ...
                      className={`w-20 h-20 sm:w-24 sm:h-24 rounded-3xl flex flex-col items-center justify-center transition-all duration-300 relative select-none cursor-pointer shadow-2xl ${
                        isCurrentSectorFinished
                          ? 'bg-gradient-to-br from-emerald-600 via-teal-700 to-indigo-800 border-2 border-emerald-300 text-white shadow-[0_0_30px_rgba(16,185,129,0.5)] hover:scale-110 active:scale-95 animate-bounce-subtle'
                          : 'bg-slate-950/90 border-2 border-slate-800 text-slate-600 hover:border-slate-700 opacity-60 cursor-pointer'
                      }`}
                    >
                      {isCurrentSectorFinished ? (
                        <div className="flex flex-col items-center">
                          <Award className="w-9 h-9 text-emerald-200 animate-pulse drop-shadow-[0_0_12px_rgba(255,255,255,0.8)]" />
                          <span className="text-[8px] font-black uppercase tracking-widest text-emerald-200 font-mono mt-1">
                            {isCertified ? 'ĐÃ ĐẠT CHUẨN' : 'MỞ KHÓA THI'}
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center">
                          <Lock className="w-7 h-7 text-slate-500" />
                          <span className="text-[7px] font-mono font-bold text-slate-500 mt-1 uppercase">
                            KHÓA (THIẾU {remainingNodes})
                          </span>
                        </div>
                      )}
                    </button>

                    {/* Info Card for 11th Gate */}
                    <div
                      onClick={() => {
                        if (isCurrentSectorFinished) {
                          setIsCertificationModalOpen(true);
                      ...
                      className={`mt-2.5 w-[185px] sm:w-[210px] px-3.5 py-2.5 rounded-2xl cursor-pointer transition-all duration-300 border backdrop-blur-md z-20 text-center flex flex-col items-center ${
                        isCurrentSectorFinished
                          ? 'bg-gradient-to-r from-emerald-950/90 to-indigo-950/90 text-emerald-200 border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)] hover:scale-105 active:scale-95'
                          : 'bg-slate-950/80 text-slate-500 border-slate-900 opacity-70'
                      }`}
                    >
                      <div className="flex items-center justify-between w-full mb-1 text-[8px] font-mono font-black uppercase tracking-widest">
                        <span className={isCurrentSectorFinished ? 'text-emerald-400' : 'text-slate-500'}>
                          🏆 CỔNG SÁT HẠCH KHU {currentSector.sectorNumber}
                        </span>
                        <span className="text-emerald-400">+300 XP</span>
                      </div>
                      <span className="text-[11px] font-black leading-tight text-white font-mono">
                        Sát Hạch Chứng Chỉ 20 Câu
                      </span>
                      <p className="text-[9px] text-slate-300 mt-1">
                        {isCurrentSectorFinished
                          ? '✓ Đã mở khóa! Bấm vào đây để thi sát hạch ngay'
                          : `Cần xong 10/10 trạm (còn ${remainingNodes} trạm)`}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Locked Holographic Overlay if ViSEF survey is not done */}
          {!hasCompletedSurvey && (
            <div className="absolute inset-0 z-30 bg-[#070b13]/85 backdrop-blur-[2.5px] flex flex-col items-center justify-center p-6 text-center">
              <div className="w-16 h-16 rounded-3xl bg-amber-500/10 border-2 border-amber-500/50 flex items-center justify-center mb-4 shadow-[0_0_35px_rgba(245,158,11,0.3)] animate-pulse">
                <Lock className="w-8 h-8 text-amber-400" />
              </div>
              <h4 className="text-lg sm:text-xl font-black text-white tracking-tight mb-2 font-mono">
                LỘ TRÌNH ĐANG CHỜ KHẢO NGHIỆM VISEF
              </h4>
              <p className="text-xs sm:text-sm text-slate-300 max-w-md leading-relaxed mb-6 font-medium">
                Toàn bộ các trạm học sẽ tự động được AI xếp lại theo mức độ rủi ro riêng của bạn và <strong>mở khóa ngay trạm đầu tiên</strong> sau khi bạn hoàn thành bài khảo nghiệm.
              </p>
              <div className="flex flex-wrap items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={() => setIsSurveyModalOpen(true)}
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 via-rose-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white font-bold text-xs sm:text-sm shadow-xl shadow-rose-500/25 transition cursor-pointer flex items-center gap-2 active:scale-95"
                >
                  <FileText className="w-4 h-4" />
                  <span>Làm Khảo Sát ViSEF (5 Phút) Để Mở Khóa</span>
                </button>
                <button
                  type="button"
                  onClick={handleQuickDemoSurvey}
                  className="px-4 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-bold transition cursor-pointer flex items-center gap-2"
                >
                  <Zap className="w-4 h-4 text-cyan-400" />
                  <span>⚡ Kích Hoạt Nhanh Bằng Dữ Liệu Mẫu</span>
                </button>
              </div>
            </div>
          )}
        ...
        {/* SECTOR COMPLETION & CERTIFICATION CALLOUT BANNER */}
        {isCurrentSectorFinished && (
          <div className="mt-6 p-5 rounded-2xl bg-gradient-to-r from-emerald-950/60 via-slate-900 to-indigo-950/60 border-2 border-emerald-500/50 shadow-2xl shadow-emerald-500/10 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                    HOÀN THÀNH KHU VỰC {currentSector.sectorNumber}
                  </span>
                  {certifiedSectors.includes(currentSector.sectorNumber) && (
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/40">
                      ✓ ĐÃ CẤP CHỨNG CHỈ VISEF
                    </span>
                  )}
                </div>
                <h4 className="text-base font-bold text-white">
                  Sát Hạch Chứng Chỉ Năng Lực Phòng Thủ Chủ Đề 1 (20 Câu Bẫy ViSEF)
                </h4>
                <p className="text-xs text-slate-300 max-w-xl">
                  Theo tiêu chuẩn ViSEF: Khi hoàn thành khu vực, học viên cần đạt điểm &gt; 50% trong bài thi 20 câu hỏi (không có từ khóa gợi ý như &quot;Không&quot;) để chính thức tốt nghiệp và lưu hồ sơ vào ViSEF.
                </p>
              </div>
            </div>
            ...
            <button
              onClick={() => setIsCertificationModalOpen(true)}
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-indigo-600 hover:from-emerald-400 hover:to-indigo-500 text-white font-black text-xs uppercase font-mono tracking-wider flex items-center space-x-2 shadow-xl shadow-emerald-500/25 shrink-0 cursor-pointer"
            >
              <ShieldCheck className="w-4 h-4" />
              <span>
                {certifiedSectors.includes(currentSector.sectorNumber)
                  ? 'Xem Lại / Thi Cải Thiện'
                  : 'Bắt Đầu Thi Chứng Chỉ Ngay'}
              </span>
            </button>
          </div>
        )}
        ...
        <div className="pt-6 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs text-slate-400 font-mono">
            {isCurrentSectorFinished ? (
              ...
              <span>Vượt hết {currentSector.nodes.length} trạm vẽ tay để hoàn thành Khu Vực và mở khóa Khu kế tiếp.</span>
            )}
          </div>

          {activeSectorIndex < sectorsSorted.length - 1 ? (
            <button
              disabled={!isSectorUnlocked(activeSectorIndex + 1)}
              onClick={() => {
                if (isSectorUnlocked(activeSectorIndex + 1)) {
                  handleSelectSectorIndex(activeSectorIndex + 1);
                }
              }}
              className={`px-6 py-3 text-xs sm:text-sm font-mono font-black rounded-xl flex items-center space-x-2 w-full sm:w-auto justify-center transition-all ${
                isSectorUnlocked(activeSectorIndex + 1)
                  ? 'border border-cyan-500/30 bg-cyan-500 text-slate-950 hover:bg-cyan-400 cursor-pointer active:translate-y-0.5 shadow-[0_0_15px_rgba(6,182,212,0.3)]'
                  : 'border border-slate-800 bg-slate-900/60 text-slate-600 cursor-not-allowed opacity-50'
              }`}
            >
              <span>
                {isSectorUnlocked(activeSectorIndex + 1)
                  ? `Lật Sang Khu Vực ${activeSectorIndex + 2}`
                  : `Khu Vực ${activeSectorIndex + 2} Đang Khóa 🔒`}
              </span>
          ...
          <div className="bg-slate-900 border-2 border-cyan-500/60 rounded-3xl max-w-md w-full shadow-2xl p-6 space-y-5 animate-scale-up relative">
            ...
            {!hasCompletedSurvey ? (
              <div className="space-y-3">
                <div className="p-3.5 rounded-2xl bg-amber-950/60 border border-amber-500/40 text-xs text-amber-200 flex items-center space-x-2.5">
                  <Lock className="w-4 h-4 flex-shrink-0 text-amber-400" />
                  <span>Cần hoàn thành bài khảo nghiệm ViSEF để mở khóa và nhận lộ trình cá nhân hóa!</span>
                </div>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full py-3 rounded-xl font-black text-xs sm:text-sm flex items-center justify-center space-x-2 shadow-lg cursor-pointer bg-gradient-to-r from-amber-500 via-rose-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white transition active:scale-95"
                >
                  <FileText className="w-4 h-4" />
                  <span>Làm Khảo Sát ViSEF Để Mở Khóa</span>
                </button>
              </div>
      ...
      {/* SECTOR TOPIC 1 CERTIFICATION MODAL */}
      <SectorTopic1CertificationModal
        ...
        preAppScore={45}
        onCertificateIssued={() => {
          // Trigger refresh of certified sectors
          setCertifiedSectors((prev) => [...prev, currentSector.sectorNumber]);
        }}
      />

      {/* VISEF SCIENTIFIC SURVEY MODAL */}
