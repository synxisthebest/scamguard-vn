                ...
                {reportDoc ? (
                  <div className="p-3 bg-slate-900 border border-slate-850 rounded-xl space-y-2.5 text-xs font-mono">
                    ...
                    <button
                      onClick={() => {
                        const content = `===============================================================
BÁO CÁO PHÁP Y KHẨN CẤP - SCAMGUARD VIETNAM EMERGENCY SOS
===============================================================
Mã chứng thư ID: ${reportDoc.docId}
Thời gian ghi nhận: ${reportDoc.timestamp}
Định vị GPS / Vị trí: ${reportDoc.gps}
Loại đe dọa lừa đảo: ${reportDoc.threatType}
Danh sách liên hệ khẩn cấp đã báo động: ${reportDoc.safetyContactsAlerted.join(', ')}
Trạng thái xác thực chứng cứ: ${reportDoc.verificationStatus}
Chữ ký số xác thực: SCAMGUARD.VN-DIGITAL-PROOF-2026

Tóm tắt sự cố:
Nạn nhân đã kích hoạt quy trình Phản Ứng Khẩn Cấp Real-time Response Suite.
Hệ thống AI đã tự động đóng băng các giao dịch nghi vấn, gửi cảnh báo SOS
tới danh bạ tin cậy và tạo hồ sơ bằng chứng số phục vụ điều tra pháp y.
===============================================================`;
                        const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `BAO_CAO_PHAP_Y_${reportDoc.docId}.txt`;
                        a.click();
                    ...
                    >
                      ...
                      <span>XUẤT HỒ SƠ BÁO CÁO (.TXT/PDF)</span>
                    </button>
                  </div>
                ) : (
                  <button
