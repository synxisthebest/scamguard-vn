...
Kiểm tra Bản Đồ Gen Phòng Thủ của bạn tại ScamGuard ngay!`;
  ...
  const radarData = useMemo(() => {
    ...
    const userPoints = dimensions.map((dim, i) => {
      ...
      const safeScore = Number.isFinite(rawScore) ? rawScore : 50;
      const ratio = Math.max(0.15, Math.min(1, safeScore / 100));
      const r = maxRadius * ratio;
      const x = center + r * Math.cos(angle);
      const y = center + r * Math.sin(angle);
      const safeX = Number.isFinite(x) ? x : center;
      const safeY = Number.isFinite(y) ? y : center;
      return { x: safeX, y: safeY, str: `${safeX.toFixed(1)},${safeY.toFixed(1)}`, score: safeScore, key: dim.key };
    });

    // Community polygon points
    const commPoints = dimensions.map((dim, i) => {
      const angle = i * angleStep - Math.PI / 2;
      const rawComm = Number(dim.communityAverage);
      const safeComm = Number.isFinite(rawComm) ? rawComm : 50;
      const ratio = Math.max(0.15, Math.min(1, safeComm / 100));
      const r = maxRadius * ratio;
      const x = center + r * Math.cos(angle);
      const y = center + r * Math.sin(angle);
      const safeX = Number.isFinite(x) ? x : center;
      const safeY = Number.isFinite(y) ? y : center;
      return { x: safeX, y: safeY, str: `${safeX.toFixed(1)},${safeY.toFixed(1)}`, score: safeComm, key: dim.key };
    });
                    ...
                    {(viewMode === 'community' || viewMode === 'compare') && (
                      <g>
                        ...
                        {radarData.commVertices.map((v, i) => {
                          const safeX = Number.isFinite(v.x) ? v.x : radarData.center;
                          const safeY = Number.isFinite(v.y) ? v.y : radarData.center;
                          return (
                            <circle
                              key={`comm-v-${i}`}
                              cx={safeX}
