          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              ...
              <div className="space-y-4 text-xs font-sans">
                ...
                {sharedStatus && (
                  <div className="p-2.5 bg-emerald-950/60 border border-emerald-800 text-emerald-300 rounded-xl text-xs font-mono font-bold animate-fadeIn">
                    ✓ {sharedStatus}
                  </div>
                )}
                ...
                <div className="flex gap-2.5">
                    ...
                    onClick={() => {
                      try {
                        ...
                        if (ctx) {
                          ...
                          ctx.fillText('SCAMGUARD VIETNAM - VISEF 2026', 80, 100);

                          ctx.fillStyle = '#ffffff';
                          ctx.font = 'bold 52px sans-serif';
                          ctx.fillText(`CHỨNG NHẬN DNA CẢNH GIÁC`, 80, 180);

                          const score = userProfile?.overallScore || userProfile?.profile?.overallScore || 85;
                          const quishing = userProfile?.profile?.dnaVector?.quishingScore || 88;
                          const deepfake = userProfile?.profile?.dnaVector?.deepfakeScore || 92;
                          const apk = userProfile?.profile?.dnaVector?.apkScore || 84;

                          ctx.fillStyle = '#94a3b8';
                          ctx.font = '28px sans-serif';
                          ctx.fillText(`Vệ binh: ${userProfile.name || 'Thành viên ScamGuard'}`, 80, 240);
                          ctx.fillText(`Chỉ số miễn dịch lừa đảo: ${score}/100 | Hạng: Vệ Binh Cao Cấp`, 80, 290);
                          ctx.fillText(`Cấp ngày: ${new Date().toLocaleDateString('vi-VN')}`, 80, 340);

                          // Vector stats
                          ctx.fillStyle = '#38bdf8';
                          ctx.font = 'bold 24px monospace';
                          ctx.fillText(`[QUISHING: ${quishing}%]  [DEEPFAKE: ${deepfake}%]  [APK/MALWARE: ${apk}%]`, 80, 420);

                          // Footer Watermark
                          ctx.fillStyle = '#06b6d4';
                          ctx.font = 'italic 22px sans-serif';
                          ctx.fillText('Được xác thực bởi Hệ thống Trí tuệ Nhân tạo Phòng chống Lừa đảo Mạng ScamGuard', 80, 520);

                          const link = document.createElement('a');
                          link.download = `SCAMGUARD_DNA_CERTIFICATE_${userProfile.name || 'USER'}.png`;
                  ...
                  <button
                    onClick={() => {
                      const shareUrl = `${window.location.origin}/share/dna-report-${userProfile.uid || 'guest'}`;
                      navigator.clipboard.writeText(shareUrl).then(() => {
                        playSuccessChime();
                        setSharedStatus('Đã sao chép liên kết chia sẻ: ' + shareUrl);
                        setTimeout(() => setSharedStatus(''), 4000);
                      }).catch(() => {
                        setSharedStatus('Liên kết: ' + shareUrl);
                      });
                    }}
