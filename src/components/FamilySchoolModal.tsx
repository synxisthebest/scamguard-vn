...
export const FamilySchoolModal: React.FC<FamilySchoolModalProps> = ({ isOpen, onClose }) => {
  ...
  const [students, setStudents] = useState([
    { name: 'Lớp 11A2 - Nhóm An Ninh Mạng', members: 38, avgScore: 84, completion: '92%', status: 'Hoạt Động' },
    { name: 'Học sinh Nguyễn Thu Trang', score: 96, drills: 15, status: 'Hạng Xuất Sắc', avgScore: 96 },
    { name: 'Học sinh Lê Hoàng Long', score: 78, drills: 8, status: 'Cần Luyện Deepfake', avgScore: 78 },
  ]);
  ...
  const handleAddMember = (e: React.FormEvent) => {
    ...
    if (activeCircle === 'family') {
      setFamilyMembers([
        ...familyMembers,
        {
          name: newName,
          score: 85,
          status: 'Đã gia nhập vòng tròn an toàn',
          tier: newRole,
        },
      ]);
    } else {
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full shadow-2xl overflow-hidden my-8 animate-scaleUp">
        ...
        <div className="p-6 sm:p-8 space-y-6 max-h-[60vh] overflow-y-auto">
          ...
          {/* Add Form */}
          <form onSubmit={handleAddMember} className="pt-4 border-t border-slate-800 space-y-3">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300 block">
              + Thêm {activeCircle === 'family' ? 'Người Thân Vòng Tròn' : 'Học Sinh / Lớp Học'}
            </span>
            <div className="flex gap-2">
              <input
                ...
                placeholder={activeCircle === 'family' ? 'Tên người thân (VD: Dì Bảy)' : 'Tên học sinh / Tên nhóm'}
              ...
              <button
                ...
                className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold text-xs rounded-xl disabled:opacity-40 transition-colors cursor-pointer"
              >
                Thêm Ngay
              </button>
            </div>
