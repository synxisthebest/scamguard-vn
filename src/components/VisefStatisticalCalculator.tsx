...
export const VisefStatisticalCalculator: React.FC = () => {
  ...
  // Live empirical values
  const groupC = liveStats?.groupMetrics?.GROUP_C_ADAPTIVE;
  const groupA = liveStats?.groupMetrics?.GROUP_A_CONTROL;
  const tTest = liveStats?.inferentialTests?.groupC_PairedTTest;
  const liveCountC = groupC?.count ?? 0;
  const liveCountA = groupA?.count ?? 0;
  const liveTotalTrials = liveCountC + liveCountA + (liveStats?.groupMetrics?.GROUP_B_NON_ADAPTIVE?.count ?? 0);

  const meanCVal = groupC?.meanPost !== undefined ? groupC.meanPost : 0;
  const meanAVal = groupA?.meanPost !== undefined ? groupA.meanPost : 0;
  const liveDelta = groupC?.meanGain !== undefined ? groupC.meanGain : (meanCVal > 0 ? +(meanCVal - meanAVal).toFixed(1) : 0);
  const liveCohensD = tTest?.cohensD !== null && tTest?.cohensD !== undefined ? tTest.cohensD : null;
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          ...
          <span className="text-xs font-mono text-purple-300 font-bold">
            {liveCohensD !== null
              ? `Cohen's d = ${liveCohensD.toFixed(2)} (Live thực nghiệm N=${liveTotalTrials})`
              : `Cohen's d = ${effectSize.toFixed(2)} (Mô hình giả thuyết G*Power)`}
          </span>
        </div>
        ...
        <div className="p-4 bg-slate-950 rounded-xl border border-slate-800">
          <svg className="w-full h-44 overflow-visible" viewBox="0 0 800 200">
            ...
            <text x="250" y="85" fill="#94a3b8" fontSize="10" fontFamily="monospace">
              Mean = {meanAVal > 0 ? `${meanAVal}đ (Live)` : '58.4đ (Mô hình H₀)'}
            </text>
            ...
            <text x="530" y="85" fill="#94a3b8" fontSize="10" fontFamily="monospace">
              Mean = {meanCVal > 0 ? `${meanCVal}đ (Live)` : '85.3đ (Mô hình H₁)'}
            </text>
            ...
            <text x="375" y="105" fill="#38bdf8" fontSize="11" fontWeight="bold" textAnchor="middle">
              Δ = {liveDelta > 0 ? `+${liveDelta}đ` : `+${(85.3 - 58.4).toFixed(1)}đ`} (d = {liveCohensD !== null ? liveCohensD.toFixed(2) : effectSize.toFixed(2)})
            </text>
          </svg>
        </div>

        <p className="text-xs text-slate-300 leading-relaxed bg-slate-950 p-3 rounded-xl border border-slate-800">
          <b>Kết luận phân tích:</b> {liveTotalTrials >= 2 && tTest?.significant ? (
            <>Dữ liệu thực nghiệm thực tế từ <b>N = {liveTotalTrials}</b> đối tượng nghiên cứu đã bác bỏ giả thuyết không ($H_0$) với mức ý nghĩa $p = {tTest?.pValue < 0.001 ? '< 0.001' : tTest?.pValue}$, khẳng định tính ưu việt của phương pháp huấn luyện phản xạ thích ứng.</>
          ) : (
            <>Mô hình toán học phân tách hai hàm mật độ xác suất chuẩn xác nhận rằng khi đạt cỡ mẫu khuyến nghị <b>N ≥ {recommendedNWithDropout}</b>, hệ thống đảm bảo độ tin cậy kiểm định {(power * 100).toFixed(0)}% (1-β) ở mức ý nghĩa α = {alpha}.</>
          )}
        </p>
      </div>
