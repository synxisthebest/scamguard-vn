...
interface SurveyDemographicsSectionProps {
  ...
  onExperiencedSectorsChange?: (val: string[]) => void;
  idPrefix?: string;
}
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="relative bg-slate-900/90 p-4 sm:p-5 rounded-2xl border border-purple-500/40 shadow-lg space-y-3.5">
        ...
        <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
          <div
            className="h-full bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 rounded-full transition-all duration-300"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Ví dụ: 10A1, 10B2, 11L1, 12H1..."
                  value={className}
          ...
          {[
            ...
            return (
              <div
                ...
                className={`p-4 sm:p-5 rounded-2xl border transition-all duration-200 cursor-pointer flex items-center justify-between gap-3 select-none active:scale-[0.98] ${
                  isSelected
                    ? 'bg-purple-600/25 border-purple-500 ring-2 ring-purple-400 shadow-xl shadow-purple-950/50 scale-[1.01]'
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <h4 className={`text-sm sm:text-base font-bold ${isSelected ? 'text-purple-100' : 'text-slate-100'}`}>
                    {g.title}
                  </h4>
                </div>

                <div
                  className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border flex items-center justify-center shrink-0 transition ${
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-800">
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
              <label className="text-white font-black text-xs sm:text-sm md:text-base flex items-center gap-1.5 sm:gap-2">
                <span>Câu 1. Bạn đã từng gặp hoặc nhận được một hình thức lừa đảo trực tuyến chưa?</span>
                <span className="text-rose-400 font-bold">*</span>
              </label>
              <span className="text-[10px] sm:text-[11px] text-slate-400">Khảo nghiệm khởi đầu: Tiền sử tiếp xúc lừa đảo mạng</span>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-[10px] sm:text-[11px] text-purple-300 font-bold">Chọn 1 đáp án</span>
          </div>
        </div>

        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
          Thông tin thu thập ẩn danh, phục vụ đối soát kết quả khảo nghiệm với 12 Khu vực trong Lộ trình Huấn luyện ViSEF.
        </p>
          ...
          {[
            {
              ...
              title: 'A. Chưa bao giờ',
              label: 'Chưa từng nhận được hoặc gặp bất kỳ tin nhắn, cuộc gọi hay tình huống lừa đảo trực tuyến nào.',
            },
            {
              id: 'SPOTTED_IN_TIME',
              icon: '🛡️',
              ...
              title: 'B. Có, nhưng chỉ gặp và nhận ra ngay',
              label: 'Đã từng gặp các chiêu trò nhưng lập tức nhận biết bẫy lừa đảo và không thực hiện bất kỳ thao tác nguy hại nào.',
            },
            {
              ...
              title: 'C. Có, từng suýt bị lừa',
              label: 'Từng bấm link nghi vấn, nhập thử thông tin hoặc nghe cuộc gọi thao túng nhưng may mắn dừng lại kịp thời.',
            },
            {
              id: 'LOST_MONEY',
              icon: '💸',
              ...
              title: 'D. Có, từng bị lừa hoặc chịu thiệt hại',
              label: 'Từng bị chiếm đoạt tiền mặt, lộ thông tin cá nhân/OTP hoặc bị khóa mất tài khoản do bẫy lừa đảo trực tuyến.',
            },
          ].map((opt) => {
        ...
        {/* Detail 12 KV Sectors Checkbox Selection */}
        {onExperiencedSectorsChange && (
          <div className="mt-5 pt-5 border-t border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-sm sm:text-base font-bold text-white flex items-center gap-2">
                <span>Tích chọn các dạng lừa đảo bạn từng gặp phải (Tự động cộng +1 mẫu cho mỗi dạng chọn):</span>
              </label>
              <span className="text-xs font-mono text-purple-300 font-bold bg-purple-500/20 px-3 py-1 rounded-lg border border-purple-500/40">
                Đã chọn: {experiencedSectors.length} dạng
              </span>
            </div>
            
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
              Mỗi mục bạn tích chọn bên dưới đại diện cho 1 trải nghiệm thực tế và sẽ trực tiếp đóng góp 1 đơn vị thống kê cho phân bổ Scam DNA:
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[420px] overflow-y-auto pr-1 custom-scrollbar">
              {[
                { id: 'KV1', title: 'KV1: SMS & Email Trúng Thưởng', desc: 'Nhận SMS/Email quà tặng, ngân hàng, nhấp link giả' },
                { id: 'KV2', title: 'KV2: Dọa Khóa SIM & Giả CA', desc: 'Cuộc gọi đe dọa khóa SIM, mạo danh Công an/Tòa án' },
                { id: 'KV3', title: 'KV3: Giao COD & CTV Online', desc: 'Đơn hàng COD lạ, làm nhiệm vụ nạp tiền hoa hồng' },
                { id: 'KV4', title: 'KV4: Bẫy Tình Pig Butchering', desc: 'Lừa tình cảm ứng dụng hẹn hò, dụ dỗ nạp tiền sàn ảo' },
                { id: 'KV5', title: 'KV5: Crypto & Ponzi Chứng Khoán', desc: 'Sàn crypto ảo, nhóm đọc lệnh Telegram kéo vốn' },
                { id: 'KV6', title: 'KV6: App VNeID APK Giả Mạo', desc: 'Tải file APK giả danh VNeID/CSGT chiếm OTP' },
                { id: 'KV7', title: 'KV7: Mã QR Quishing Thanh Toán', desc: 'Quét mã QR dán đè tại bàn ăn / web chuyển tiền giả' },
                { id: 'KV8', title: 'KV8: Sự Cố Khẩn Giờ Vàng', desc: 'Báo tin người thân cấp cứu gấp giục chuyển tiền' },
                { id: 'KV9', title: 'KV9: Deepfake AI Video Call', desc: 'Cuộc gọi video ghép mặt & giả giọng bạn thân' },
                { id: 'KV10', title: 'KV10: Web3 & Smart Contract', desc: 'Ký ví Web3 độc hại, bẫy airdrop tài sản số' },
                { id: 'KV11', title: 'KV11: Juice Jacking & Wifi Độc', desc: 'Wifi công cộng giả mạo, trạm sạc USB lấy dữ liệu' },
                { id: 'KV12', title: 'KV12: Bẫy Lừa Đảo Kép', desc: 'Dịch vụ thu hồi tiền lừa đảo lần 2 giả luật sư' },
              ].map((kv) => {
                const isChecked = experiencedSectors.includes(kv.id);
                return (
                  <label
                    key={kv.id}
                    onClick={(e) => {
                      e.preventDefault();
                      let next = [...experiencedSectors];
                      if (isChecked) {
                        next = next.filter((item) => item !== kv.id);
                      } else {
                        next.push(kv.id);
                        if (pastLossOrNearMiss === 'NEVER') {
                          onPastLossOrNearMissChange('SPOTTED_IN_TIME');
                        }
                      }
                      onExperiencedSectorsChange(next);
                    }}
                    className={`p-3.5 sm:p-4 rounded-2xl border text-xs sm:text-sm cursor-pointer transition flex items-start gap-3 select-none active:scale-[0.99] ${
                      isChecked
                        ? 'bg-purple-600/20 border-purple-500 text-white ring-2 ring-purple-500/50 shadow-md'
                  ...
                  >
                    <input
                      ...
                      checked={isChecked}
                      onChange={() => {}}
                      className="mt-1 w-4 h-4 sm:w-5 sm:h-5 accent-purple-500 rounded shrink-0 cursor-pointer"
                    />
                    <div className="space-y-1">
                      <span className="font-bold text-slate-100 block text-xs sm:text-sm">{kv.title}</span>
                      <span className="text-xs text-slate-300 block leading-relaxed">{kv.desc}</span>
                    </div>
                  </label>
                );
              })}
            ...
            <div className="pt-2">
              <label
                onClick={(e) => {
                  e.preventDefault();
                  onExperiencedSectorsChange([]);
                  onPastLossOrNearMissChange('NEVER');
                }}
                className={`p-3.5 sm:p-4 rounded-2xl border text-xs sm:text-sm cursor-pointer transition flex items-center gap-3 select-none active:scale-[0.99] ${
                  pastLossOrNearMiss === 'NEVER' && experiencedSectors.length === 0
                    ? 'bg-emerald-600/20 border-emerald-500 text-emerald-200 ring-2 ring-emerald-500/50 font-bold'
                    : 'bg-slate-900/40 border-slate-800 text-slate-400 hover:border-slate-700'
                }`}
              >
                <span className="text-lg sm:text-xl">⚪</span>
                <span className="text-xs sm:text-sm font-medium">Tôi CHƯA TỪNG gặp hoặc bị lừa qua bất kỳ dạng nào (Không tăng đơn vị nào)</span>
              </label>
            </div>
          </div>
        )}
