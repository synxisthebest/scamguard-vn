...
export function AuthModal({
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let result = await loginWithFirebase(emailToUse, loginPassword);
      ...
      // 2. Fallback to server local auth endpoint
      ...
      playAlertWarning();
      setErrorMsg(result.error || data.error || 'Email hoặc mật khẩu không chính xác.');
    } catch (err: any) {
      playAlertWarning();
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let result = await registerWithFirebase({
        name: regName,
      ...
      if (result.success && result.user) {
        ...
        setSuccessMsg(`Tạo tài khoản Firebase thành công! Xin chào ${result.user.name}.`);
        localStorage.setItem('scamguard_token', result.user.token);
        playSuccessChime();
        setTimeout(() => {
          onLoginSuccess(result.user!);
      ...
      // 2. Fallback to server local registration
      ...
      playAlertWarning();
      setErrorMsg(result.error || data.error || 'Đăng ký không thành công.');
    } catch (err: any) {
      playAlertWarning();
  ...
  const handleSocialAuth = async (provider: 'google' | 'facebook' | 'github') => {
    ...
    try {
      ...
      // If Firebase Popup was closed, blocked, or failed, fallback to server social auth
      const res = await fetch('/api/auth/social', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider, mode: regMode }),
      });
      const data = await res.json();
      if (res.ok && data.success && data.user) {
        setSuccessMsg(`Đăng nhập thành công qua ${provider.toUpperCase()}!`);
      ...
      playAlertWarning();
      setErrorMsg(result.error || data.error || 'Đăng nhập mạng xã hội không thành công.');
    } catch (err: any) {
      playAlertWarning();
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[92vh] flex flex-col my-auto"
