...
export interface CommunityScamDna {
  ...
  sectorDistribution?: Array<{ key: string; name: string; count: number; pct: number }>;
...
export interface CommunitySurveySubmission {
  ...
  surveyResponses: {
    ...
    experienceAnswers?: Record<string, string>;
...
export interface PostAppCertificationRecord {
  id: string;
  participantName: string;
  anonymousCode: string;
  demographicGroup: SurveyDemographicGroup;
...
export interface PrePostComparisonMetrics {
  ...
  community: {
    ...
    demographicGains?: {
      STUDENT?: number | null;
      ELDERLY?: number | null;
      OFFICE_WORKER?: number | null;
      BUSINESS_OWNER?: number | null;
      TEACHER_JUDGE?: number | null;
    };
    domainTransformations: Array<{
      domainName: string;
      preVulnerabilityPct: number;
      postVulnerabilityPct: number;
      gainPct: number;
    }>;
  };
}
