...
async function startServer() {
  ...
  // Post-App Certification Dataset (Learners who finished sectors and passed Topic 1 Exam)
  app.get('/api/research/post-app-certifications', (req, res) => {
    try {
      const certifications = getAllPostAppCertifications();
      res.setHeader('Content-Type', 'application/json');
      res.json({ certifications, total: certifications.length });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Lỗi khi lấy dữ liệu chứng chỉ sau học' });
    }
  });

  // Record a new Post-App Certification Exam result
  app.post('/api/research/post-app-certifications', (req, res) => {
    try {
      const record = recordPostAppCertificationSubmission(req.body);
      res.setHeader('Content-Type', 'application/json');
      res.json({ success: true, record });
    } catch (err: any) {
      res.status(400).json({ error: err.message || 'Lỗi khi lưu kết quả chứng chỉ sau học' });
    }
  });

  // Pre vs Post Learning Comparison (Individual & Community ViSEF impact)
  app.get('/api/research/pre-post-comparison', (req, res) => {
    try {
      const analysis = getPrePostComparisonAnalysis();
      res.setHeader('Content-Type', 'application/json');
      res.json(analysis);
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Lỗi khi tính toán so sánh trước/sau' });
    }
  });
